...
public class FileSystemStorageService implements StorageService {
    ...
    public void store(String metadata) {
        if (metadata.length() ==0 || metadata ==null) {
            return;
        }
        try( PrintWriter out = new PrintWriter(this.rootDir.resolve(metadata.substring(0, Math.min(4, metadata.length() - 1))).toString())  ){
    ...
    public Stream<Path> loadAll() {
        try {
            return Files.walk(this.rootDir, 1)
                    ...
                    .map(path -> this.rootDir.relativize(path));
        } catch (IOException e) {
